...
async def connect_and_send():
    while 1:
        try:
            ...
            if not device:
                raise BleakError(f"No device named " + PRINTER_NAME + " could be found.")
...
def create_text(text):
    ...
    font = PIL.ImageFont.truetype("static/fonts/" + font_name, font_size)
...
def http_server_upload():
    global text_queue, ready, device, status, font_size, font_name
    if request.method == 'POST':
        ...
        try:
            ...
            print("Could not get data from json, trying text instead")
            text = request.form["text"]
            font_size = 30
            try:
                font_size = int(request.form["size"])
                if font_size < 10 or font_size > 100:
                    raise "Invalid font size"
            except:
            ...
            if font == "Cat paw":
                ...
                font_name = "Cute Meow.otf"
        print(text)
        if text is not None:
            text_queue.append(text)
            return 'Sent to printer queue <a href="/index.html">Again</a> <a href="/status">Status</a>'
        else:
            return "Nothing :("
    else:
